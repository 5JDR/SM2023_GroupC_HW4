...
(c) If $\lambda = \inf$ and $m = 2$, then we have that $\hat{g} = arg \min_g (\sum_{i=1}^n (y_i − g(x_i))^2 + \lambda \int [g^{(2)}(x)]^2 dx)$, which is the least squares fit with a penalty on the second derivative. The penalty term is so high that the fit is a straight line.
(d) If $\lambda = \inf$ and $m = 3$, then we have that $\hat{g} = arg \min_g (\sum_{i=1}^n (y_i − g(x_i))^2 + \lambda \int [g^{(3)}(x)]^2 dx)$, which is the least squares fit with a penalty on the third derivative. The penalty term is so high that the fit is a straight line.
...
```{r}
fit_2 <- lm(accel ~ poly(time, 3), data = df)

termplot(fit_2, partial.resid = TRUE, se = FALSE)
```

We can notice that the polynomial fit is much worse than the penalized regression spline fit. This is because it can't manage to capture the large changes in the function.
...
```
We can watch $R^2$ but we have no information about smoothness, we could use GCV value? How to obtain it from lm?

```{r}
# It looks heteroskedastic
plot(mcycle$time, fit_1$residuals)
```
...
The model results similar to the penalized regression spline fit (fit_1), but it's not as smooth as the penalized regression spline fit. The penalized regression spline fit looks the best model for this data.
